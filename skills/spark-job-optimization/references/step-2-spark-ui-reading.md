...
- 对于明显重要或运行超过 30 分钟的 job / stage，要补抓对应详情页和 task 级信息，不能只停留在 overview
  - 如果 task 数量过多，优先按 `shuffle read` / `shuffle write` 排序，只抓当前第一页 task 的完整信息，不需要把全部 task 页面都采下来
...
- 关键 job / stage 详情页和 task 级信息；表格块尽量保留为 Markdown table。task 数量过多时，只收集排序后的第一页 task 详情，不需要全量 task 列表。
- 运行时的 `已确认` / `待确认`
- 如果存在优化后快照，还要保留前后对比项
