...
- 由子 skill 在能直接确认时顺手标出冗余字段、小文件、重复执行、量级异常等候选项；如果发现排序字段或其他计算字段下游未使用、也没有写入下游表，要把“前置删除计算逻辑”或“前置置空 / 置默认值”作为候选优化方向输出给第 4 步
- 如果源码表现为“先大 scan，再做大量解析 / 映射 / 标准化 / 分支归一化”，并且源表是增量表或能按 `pt_date` 增量维护，要把“把解析逻辑前置到增量中间表”作为候选优化方向输出给第 4 步，而不是只停留在主链路局部减重
