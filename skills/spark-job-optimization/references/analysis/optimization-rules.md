...
- 每个分析案例放在 `<business_repo_root>/tmp/spark-job-optimization/<case_name>/`。
- 源码 zip 放在 `<business_repo_root>/tmp/spark-job-optimization/<case_name>/input/source.zip`，或者解压到 `.../input/source/`。
- Spark UI 导出放在 `.../input/spark_ui/`。
- 集群和上游表上下文放在 `.../input/context/`（如有）。
- event log、SQL plan、截图、运行备注放在同一个 case 工作区里。
- 诊断报告和变更产物输出到 `output/`，该目录位于 `<business_repo_root>/tmp/spark-job-optimization/<case_name>/` 下。
