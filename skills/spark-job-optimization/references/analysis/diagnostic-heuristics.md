...
- 如果 scan 很大，但从这段 scan 到第一个 shuffle 之间，`read / input` 明显降下来了，说明这段链路很快形成了可复用的中间态；这类问题优先考虑前置中间表或中间快照，把重扫描固化下来。
- 如果 scan 之后紧跟大量解析 / 映射 / 标准化 / 分支归一化，而且源表本身就是增量表或可按日期增量维护，要优先考虑把这些解析逻辑前置到增量中间表里沉淀，避免每次主链路重复做同一批规则转换。
...
- 对于上游出现但下游不消费的排序字段、中间列，优先直接删除其计算逻辑，而不是仅在结果表末端清理；如果短期内不方便删除，也可以考虑在链路一开始就把对应字段置空 / 置默认值，避免继续参与后续计算，但前提是不会影响中间排序、过滤或写入口径。
...
- `P2`
  ...
  - 当 scan 到第一个 shuffle 之前的 `read / input` 明显收缩时，优先考虑把这段前置成中间表或中间快照，避免每次都从头扫到首个 shuffle
