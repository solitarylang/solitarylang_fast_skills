...
- 如果 scan 很大，但从这段 scan 到第一个 shuffle 之间，`read / input` 明显收缩，说明这段链路很快形成了可复用的中间态；这类 stage 优先标成前置中间表或中间快照候选。
- 如果 scan 很大之后紧跟着大量解析 / 映射 / 标准化 / 分支归一化逻辑，而且输入本身已经是增量表或可按 `pt_date` 增量维护的表，要优先考虑把这段解析逻辑沉淀到增量中间表里，避免主链路每次重复做同一批解析和映射。
