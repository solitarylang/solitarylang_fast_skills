...
5. 对于运行时间超过 30 分钟的 job 或 stage，必须继续点击对应 stage，采集 task 级别的更细运行信息，包括 task duration 分布、失败 task、shuffle、spill、长尾和是否存在 skew。
   - 如果进入 stage 后发现 task 数量过多，优先按 `shuffle read` / `shuffle write` 排序后，只采集当前第一页 task 的完整信息，不要尝试把所有 task 页面都抓完。
...
10. 对于明显重要或运行超过 30 分钟的 job / stage，要补抓对应详情页和 task 级信息，不能只停留在 overview。
    - task 数量过多时，只保留排序后的第一页 task 明细；优先看 `shuffle read` / `shuffle write` 相关的排序结果，不要把全部 task 都展开采集。
