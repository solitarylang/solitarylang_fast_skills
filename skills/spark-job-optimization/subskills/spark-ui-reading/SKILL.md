...
- 不能提前进入根因排序
- 不能把推断写成已确认结论
- 要解释 Spark 运行参数和运行日志是否一致；如果参数配置和现象冲突，把它记录为调参候选，而不是直接当作根因
...
5. 再结合 `references/spark-parameter-heuristics.md` 解释 Spark 运行参数，判断是否存在可验证的调参候选。
6. 只记录直接证据，并保持 stage 到源码的对齐关系明确。
7. 停止在根因排序之前，把根因分析留给父级优化 skill。
...
`references/spark-parameter-heuristics.md`：Spark 运行参数解释和调参经验。  
